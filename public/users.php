<?php
/**
 * Gestión de Usuarios - VERSIÓN CORREGIDA 2.0
...
// ============================================================================
// PROCESAR ACCIONES AJAX CON VALIDACIÓN MEJORADA
// ============================================================================

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    header('Content-Type: application/json');
    
    // VALIDACIÓN 1: Verificar que existe el parámetro action
    if (!isset($_POST['action']) || empty($_POST['action'])) {
        echo json_encode(['success' => false, 'message' => 'Acción no especificada']);
        exit;
    }
    
    // VALIDACIÓN 2: Verificar token CSRF
    $csrf_token = $_POST['csrf_token'] ?? '';
    if (!verifyCsrfToken($csrf_token)) {
        logError("CSRF token inválido en users.php - User: " . $user['id'] . " - IP: " . ($_SERVER['REMOTE_ADDR'] ?? 'unknown'));
        echo json_encode(['success' => false, 'message' => 'Token CSRF inválido. Recarga la página e intenta nuevamente.']);
        exit;
    }
    ...
    try {
        ...
        switch ($action) {
            ...
            case 'create_user':
                ...
                if ($password_validation !== true) {
                    throw new Exception('Contraseña débil: ' . implode(', ', $password_validation));
                }
                ...
                $check_stmt = $db->prepare("SELECT id FROM usuarios WHERE username = ? LIMIT 1");
                ...
                if (!empty($email)) {
                    $check_email_stmt = $db->prepare("SELECT id FROM usuarios WHERE email = ? AND email != '' LIMIT 1");
                ...
                if ($result) {
                    ...
                    logActivity($user['id'], 'create_user', "Usuario creado: $username (ID: $newUserId, Rol: $rol)");
                    echo json_encode([
                        'success' => true, 
            ...
            case 'update_user':
                ...
                $check_stmt = $db->prepare("SELECT id FROM usuarios WHERE username = ? AND id != ? LIMIT 1");
                ...
                if (!empty($email)) {
                    $check_email_stmt = $db->prepare("SELECT id FROM usuarios WHERE email = ? AND id != ? AND email != '' LIMIT 1");
                ...
                if ($result) {
                    logActivity($user['id'], 'update_user', "Usuario actualizado ID: $user_id ($username, Rol: $rol)");
        ...
        logError("Error en gestión de usuarios: " . $e->getMessage() . " - User: " . $user['id']);
...
<body class="bg-gray-100">
                ...
                <div class="flex items-start">
                    ...
                    <div>
                        ...
                        <div class="text-sm text-blue-700 mt-1 space-y-1">
                            ...
                            <p><strong>Vendedores:</strong> Solo pueden realizar ventas en su tienda asignada</p>
                        </div>
                    </div>
                </div>
            </div>
                            ...
                            <?php else: ?>
                                <?php foreach($users as $usr): ?>
                                    <tr class="hover:bg-gray-50 transition-colors user-row" 
                                        data-username="<?php echo htmlspecialchars(strtolower($usr['username']), ENT_QUOTES, 'UTF-8'); ?>"
                                        data-nombre="<?php echo htmlspecialchars(strtolower($usr['nombre']), ENT_QUOTES, 'UTF-8'); ?>"
                                        data-email="<?php echo htmlspecialchars(strtolower($usr['email'] ?? ''), ENT_QUOTES, 'UTF-8'); ?>"
                                        data-rol="<?php echo htmlspecialchars($usr['rol'], ENT_QUOTES, 'UTF-8'); ?>"
                                        data-activo="<?php echo htmlspecialchars($usr['activo'], ENT_QUOTES, 'UTF-8'); ?>">
                                        <td class="px-4 py-4">
                                            <div class="flex items-center">
                                                ...
                                                <div>
                                                    <p class="font-medium text-gray-900"><?php echo htmlspecialchars($usr['nombre'], ENT_QUOTES, 'UTF-8'); ?></p>
                                                    <p class="text-sm text-gray-600">@<?php echo htmlspecialchars($usr['username'], ENT_QUOTES, 'UTF-8'); ?></p>
                                                    <?php if ($usr['email']): ?>
                                                        <p class="text-xs text-gray-500"><?php echo htmlspecialchars($usr['email'], ENT_QUOTES, 'UTF-8'); ?></p>
                                                    <?php endif; ?>
                                                </div>
                                            </div>
                                        </td>
                                        <td class="px-4 py-4 text-sm text-gray-900">
                                            ...
                                            <?php elseif ($usr['tienda_nombre']): ?>
                                                <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                                                    <?php echo htmlspecialchars($usr['tienda_nombre'], ENT_QUOTES, 'UTF-8'); ?>
                                                </span>
                                            <?php else: ?>
                                        ...
                                        <td class="px-4 py-4 text-sm text-gray-900">
                                            <div>
                                                ...
                                                <p class="text-xs text-gray-500">
                                                    <?php echo intval($usr['total_ventas']); ?> ventas realizadas
                                                </p>
                                            </div>
                                        </td>
                                        <td class="px-4 py-4">
                                            <div class="flex items-center gap-2">
                                                <button onclick='openEditModal(<?php echo json_encode($usr, JSON_HEX_APOS | JSON_HEX_QUOT); ?>)' 
                                                ...
                                                <button onclick="openPasswordModal(<?php echo $usr['id']; ?>, '<?php echo htmlspecialchars($usr['nombre'], ENT_QUOTES, 'UTF-8'); ?>')" 
                                                ...
                                                <?php if ($usr['id'] != $user['id']): ?>
                                                    <button onclick="deleteUser(<?php echo $usr['id']; ?>, '<?php echo htmlspecialchars($usr['nombre'], ENT_QUOTES, 'UTF-8'); ?>')" 
                    ...
                    <select id="tienda_id" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                        ...
                        <?php if (is_array($tiendas)): ?>
                            <?php foreach($tiendas as $tienda): ?>
                                <option value="<?php echo $tienda['id']; ?>"><?php echo htmlspecialchars($tienda['nombre'], ENT_QUOTES, 'UTF-8'); ?></option>
                            <?php endforeach; ?>
                        <?php endif; ?>
                    </select>
    ...
    <script>
        ...
        function openEditModal(userData) {
            ...
            document.getElementById('userId').value = sanitizeHTML(userData.id);
            document.getElementById('username').value = sanitizeHTML(userData.username);
            document.getElementById('nombre').value = sanitizeHTML(userData.nombre);
            document.getElementById('email').value = sanitizeHTML(userData.email || '');
            document.getElementById('tienda_id').value = sanitizeHTML(userData.tienda_id || '');
            document.getElementById('rol').value = sanitizeHTML(userData.rol);
        ...
        function openPasswordModal(userId, userName) {
            ...
            document.getElementById('passwordUserName').textContent = sanitizeHTML(userName);
        ...
        function showNotification(message, type = 'info') {
            ...
            notification.innerHTML = `
                <div class="flex items-center">
                    <div class="flex-1">${sanitizeHTML(message)}</div>
