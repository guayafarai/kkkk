        ...
        switch ($_POST['action']) {
            case 'add_product':
                ...
                if ($stmt->execute([$codigo ?: null, $nombre, $descripcion, $categoria_id, $tipo, $marca, 
                    ...
                    logActivity($user['id'], 'add_product', "Producto: $nombre (ID: $product_id)");
...
if (!empty($search)) {
    $where_conditions[] = "(p.nombre LIKE ? OR p.codigo_producto LIKE ? OR p.marca LIKE ?)";
    $search_param = "%$search%";
...
<main class="<?php echo $navbar_exists ? 'page-content' : 'container mx-auto'; ?> p-6">
    ...
    <div class="bg-white rounded-lg shadow-sm p-4 mb-6">
        <form method="GET" class="flex flex-wrap gap-4">
            ...
            <select name="categoria" class="px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500">
            ...
            <select name="tienda" class="px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500">
...
function openAddProductModal(){isEditMode=false;document.getElementById('productModalTitle').textContent='Agregar Producto';document.getElementById('productForm').reset();document.getElementById('productId').value='';document.getElementById('statusField').classList.add('hidden');document.getElementById('productModal').classList.add('show')}

function openEditProductModal(product){isEditMode=true;document.getElementById('productModalTitle').textContent='Editar Producto';document.getElementById('productId').value=product.id;document.getElementById('nombre').value=product.nombre||'';document.getElementById('codigo_producto').value=product.codigo_producto||'';document.getElementById('categoria_id').value=product.categoria_id||'';document.getElementById('tipo').value=product.tipo||'';document.getElementById('marca').value=product.marca||'';document.getElementById('modelo_compatible').value=product.modelo_compatible||'';document.getElementById('precio_venta').value=product.precio_venta||'';document.getElementById('precio_compra').value=product.precio_compra||'';document.getElementById('minimo_stock').value=product.minimo_stock||5;document.getElementById('descripcion').value=product.descripcion||'';document.getElementById('activo').checked=product.activo==1;document.getElementById('statusField').classList.remove('hidden');document.getElementById('productModal').classList.add('show')}

function closeProductModal(){document.getElementById('productModal').classList.remove('show')}

function saveProduct(){const formData=new FormData();formData.append('action',isEditMode?'update_product':'add_product');formData.append('csrf_token',CSRF_TOKEN);if(isEditMode)formData.append('product_id',document.getElementById('productId').value);formData.append('nombre',document.getElementById('nombre').value);formData.append('codigo_producto',document.getElementById('codigo_producto').value);formData.append('categoria_id',document.getElementById('categoria_id').value);formData.append('tipo',document.getElementById('tipo').value);formData.append('marca',document.getElementById('marca').value);formData.append('modelo_compatible',document.getElementById('modelo_compatible').value);formData.append('precio_venta',document.getElementById('precio_venta').value);formData.append('precio_compra',document.getElementById('precio_compra').value);formData.append('minimo_stock',document.getElementById('minimo_stock').value);formData.append('descripcion',document.getElementById('descripcion').value);if(isEditMode&&document.getElementById('activo').checked)formData.append('activo','1');fetch('products.php',{method:'POST',body:formData}).then(r=>r.json()).then(data=>{if(data.success){showNotification(data.message,'success');setTimeout(()=>location.reload(),1000)}else{showNotification(data.message,'error')}}).catch(()=>showNotification('Error de conexión','error'))}

function deleteProduct(id,name){if(!confirm(`¿Eliminar "${name}"?`))return;const formData=new FormData();formData.append('action','delete_product');formData.append('product_id',id);formData.append('csrf_token',CSRF_TOKEN);fetch('products.php',{method:'POST',body:formData}).then(r=>r.json()).then(data=>{if(data.success){showNotification(data.message,'success');setTimeout(()=>location.reload(),1000)}else{showNotification(data.message,'error')}})}

function openStockModal(productId,tiendaId,productName,currentStock){document.getElementById('stockProductoId').value=productId;document.getElementById('stockTiendaId').value=tiendaId;document.getElementById('stockProductName').textContent=productName;document.getElementById('stockCurrent').textContent=currentStock;document.getElementById('nueva_cantidad').value=currentStock;document.getElementById('motivo_ajuste').value='';document.getElementById('stockModal').classList.add('show')}

function closeStockModal(){document.getElementById('stockModal').classList.remove('show')}

function adjustStock(){if(!document.getElementById('motivo_ajuste').value){showNotification('Selecciona un motivo','warning');return}const formData=new FormData();formData.append('action','adjust_stock');formData.append('csrf_token',CSRF_TOKEN);formData.append('producto_id',document.getElementById('stockProductoId').value);formData.append('tienda_id',document.getElementById('stockTiendaId').value);formData.append('nueva_cantidad',document.getElementById('nueva_cantidad').value);formData.append('motivo',document.getElementById('motivo_ajuste').value);fetch('products.php',{method:'POST',body:formData}).then(r=>r.json()).then(data=>{if(data.success){showNotification(data.message,'success');closeStockModal();setTimeout(()=>location.reload(),1000)}else{showNotification(data.message,'error')}})}

function openAddStockModal(){document.getElementById('addStockForm').reset();<?php if(!hasPermission('admin')): ?>document.getElementById('add_tienda_id').value=<?php echo $user['tienda_id']; ?>;<?php endif; ?>document.getElementById('addStockModal').classList.add('show')}

function closeAddStockModal(){document.getElementById('addStockModal').classList.remove('show')}

function addStock(){const producto=document.getElementById('add_producto_id').value;const tienda=document.getElementById('add_tienda_id').value;const cantidad=document.getElementById('add_cantidad').value;if(!producto||!tienda||!cantidad){showNotification('Completa todos los campos','warning');return}const formData=new FormData();formData.append('action','add_stock');formData.append('csrf_token',CSRF_TOKEN);formData.append('producto_id',producto);formData.append('tienda_id',tienda);formData.append('cantidad',cantidad);formData.append('precio_unitario',document.getElementById('add_precio_unitario').value);formData.append('motivo',document.getElementById('add_motivo').value);fetch('products.php',{method:'POST',body:formData}).then(r=>r.json()).then(data=>{if(data.success){showNotification(data.message,'success');closeAddStockModal();setTimeout(()=>location.reload(),1000)}else{showNotification(data.message,'error')}})}

function showNotification(message,type='info'){const colors={'success':'bg-green-500','error':'bg-red-500','warning':'bg-yellow-500','info':'bg-blue-500'};const notification=document.createElement('div');notification.className=`notification ${colors[type]} text-white px-6 py-4 rounded-lg shadow-lg`;notification.innerHTML=`<div class="flex items-center justify-between"><span>${message}</span><button onclick="this.parentElement.parentElement.remove()" class="ml-4"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg></button></div>`;document.body.appendChild(notification);setTimeout(()=>notification.classList.add('show'),100);setTimeout(()=>{notification.classList.remove('show');setTimeout(()=>notification.remove(),300)},5000)}

document.getElementById('tipo')?.addEventListener('change',function(){const categoriaSelect=document.getElementById('categoria_id');if(!categoriaSelect)return;const selectedTipo=this.value;const options=categoriaSelect.querySelectorAll('option[data-tipo]');options.forEach(option=>{if(!selectedTipo||option.dataset.tipo===selectedTipo){option.style.display=''}else{option.style.display='none'}});const selectedOption=categoriaSelect.options[categoriaSelect.selectedIndex];if(selectedOption&&selectedOption.dataset.tipo&&selectedOption.dataset.tipo!==selectedTipo){categoriaSelect.value=''}});
